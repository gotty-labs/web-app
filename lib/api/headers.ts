/**
 * Builder for the mandatory `gt-*` request headers (§1.2). These are validated by
 ...
 *  - `gt-origin`            → always WEB.
 *  - `gt-platform-version`  → the app version from package.json (single source).
 *  - `gt-platform-buildnumber` → hardcoded "1" (web has no store build number).
 *  - `gt-language`          → en|es, defaults to es; wired to the language selector later.
 *
...
export const DEFAULT_LANGUAGE: GtLanguage = defaultLocale
...
export function buildHeaders(options: HeaderOptions = {}): Headers {
  ...
  headers.set('gt-origin', PLATFORM.origin)
  headers.set('gt-platform-version', PLATFORM.version)
