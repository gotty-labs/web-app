...
// forked from design-sync lib/previews.mjs — reset the esbuild service every
// N sequential preview builds (esbuild's shared service deadlocks at 0% CPU
// after ~23 build() calls in one process on this repo; see .design-sync/NOTES.md).
...
export async function buildPreviews({ components, previewDir, genDir, OUT, reactShim, NODE_MODULES, pathsPlugin, importPlugins, loaders }) {
  ...
  // The shared esbuild service wedges (0% CPU) after ~23 sequential build()
  // calls in one process on this repo. Reset it every RESET_EVERY builds —
  // stop() tears the service down; the next build() transparently respawns it.
  const RESET_EVERY = 15;
  let sinceReset = 0;
  for (const c of components) {
    const owned = join(previewDir, `${c.name}.tsx`);
    const entry = existsSync(owned) ? owned : join(genDir, `${c.name}.tsx`);
    if (!existsSync(entry)) continue;
    if (sinceReset >= RESET_EVERY) { await stop(); sinceReset = 0; }
    sinceReset++;
    try {
      await build({
